...
try: 
    ...
    for a in a_tags:
        try :
            ...
            descs = []
            for e in elements[1:]:
                if len(e.findChildren("h3")) and len(e.findChildren("p")):
                    titles.append(e.findChildren("h3")[0].text)
                    descs.append(e.findChildren("p")[0].text)        
            watches_count = watches_count + 1 
            watches[watches_count] = [url, title] + descs
            items_df = pd.DataFrame.from_dict(watches, orient='index', columns = ['URL' , 'Title'] + titles)
